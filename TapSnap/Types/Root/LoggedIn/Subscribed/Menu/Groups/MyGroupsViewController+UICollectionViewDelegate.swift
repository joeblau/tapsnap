...
extension MyGroupsViewController: UICollectionViewDelegate {
    ...
    private func buildContextMenu() -> UIMenu {
        ...
        let rename = UIAction(title: L10n.titleRename, image: UIImage(systemName: "square.and.pencil")) { _ in
            let alert = RenameGroupViewController(title: L10n.titleRenameGroup, message: nil, preferredStyle: .alert)
            alert.groupName = selectedGroup[GroupKey.name] as? String ?? "-"
            self.present(alert, animated: true)
        }

        let image = UIAction(title: L10n.titleImage, image: UIImage(systemName: "photo")) { _ in
            let imagePickerViewController = UIImagePickerController()
            imagePickerViewController.delegate = self
            self.present(imagePickerViewController, animated: true, completion: nil)
        }

        let share = UIAction(title: L10n.titleShare, image: UIImage(systemName: "square.and.arrow.up")) { _ in
            // Share
        }

        let delete = UIAction(title: L10n.titleDelete, image: UIImage(systemName: "trash"), attributes: .destructive) { _ in
            let alert = DeleteGroupViewController(title: L10n.titleDeleteGroup, message: L10n.bodyDeleteGroup, preferredStyle: .alert)
            self.present(alert, animated: true)
        }
