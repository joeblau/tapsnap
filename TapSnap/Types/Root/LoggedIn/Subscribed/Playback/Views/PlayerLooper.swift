    ...
    override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey: Any]?, context: UnsafeMutableRawPointer?) {
        if context == &ObserverContexts.isLooping {
            if let loopingStatus = change?[.newKey] as? Bool, !loopingStatus {
                os_log("%@", log: .avFoundation, type: .error, "Looping ended due to an error")
            }
            ...
            if numberOfTimesToPlay > 0, playerLooper.loopCount >= numberOfTimesToPlay - 1 {
                os_log("%@", log: .avFoundation, type: .error, "Exceeded loop limit of \(numberOfTimesToPlay) and disabling looping")
                stopObserving()
                playerLooper.disableLooping()
            }
        } else {
