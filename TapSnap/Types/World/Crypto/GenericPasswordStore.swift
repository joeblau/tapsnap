...
struct GenericPasswordStore {
    ...
    func storeKey<T: GenericPasswordConvertible>(_ key: T, account: String) throws {
        ...
        guard status == errSecSuccess else {
            throw KeyStoreError(L10n.errorKeystoreUnableToStore(status.message))
        }
    }
    ...
    func readKey<T: GenericPasswordConvertible>(account: String) throws -> T? {
        ...
        case let status: throw KeyStoreError(L10n.errorKeystoreKeychainReadFailed(status.message))
        }
    }
    ...
    func roundTrip<T: GenericPasswordConvertible>(_ key: T) throws -> T {
        ...
        guard let key: T = try readKey(account: account) else {
            throw KeyStoreError(L10n.errorKeystoreFailedToLocateStoredKey)
        }
        return key
    }
    ...
    func deleteKey(account: String) throws {
        ...
        case let status:
            ...
            throw KeyStoreError(L10n.errorKeystoreUnexpectedDeletion(status.message))
        }
    }
}
